...
function addTODO() {
  ...
  const newTodo = {
    ...
    createdAt: currentDate // Add the creation date to the todo object
  };
  localStorage.setItem('chill31-nuxt-todo-data', JSON.stringify([...todos.value, newTodo]));
...
<template>
  ...
  <div
    ...
    <UButton class="mt-16 flex items-center justify-center gap-0 cancel-button" icon="i-mdi-close-circle" size="x8" color="red"
  v-if="todoFormVisible" v-on:click="toggleTODOForm()">
  Cancel
</UButton>
    <div class="flex flex-col gap-2 items-start justify-start w-[25rem] max-w-[65vw]" v-if="todoFormVisible">
      ...
      <UTextarea id="form-input" class="w-full" color="gray" variant="outline" size="xl" v-model="formValue"
        placeholder="enter todo discription..."></UTextarea>
    ...
    <div class="flex w-screen flex-wrap items-start justify-center gap-4">
      <div v-for="todo, k in todos">
        ...
        <div
          ...
          <p class="text-sm text-gray-500">{{ todo.createdAt }}</p>
          <div
...
<style scoped>
.cancel-button {
  background-color: #ff3d00; /* Orange color */
  color: white; /* White text color */
  border-radius: 8px; /* Rounded corners */
  padding: 12px 24px; /* Increase padding for better touch target */
  transition: background-color 0.3s ease; /* Smooth transition on hover */

  /* Add hover effect */
  &:hover {
    background-color: #ff6d00; /* Darker orange color on hover */
  }
}
</style>
